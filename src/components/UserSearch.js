...
class UserSearch extends Component {
	...
	selectUser = (ev) => {
		...
		const filteredUsers = this.state.autoCompleteUsers.filter((user) => {
						...
						<ul className="menu">
							{
								this.state.autoCompleteUsers.map((user) => {
									return <UserSearchListItem id={user._id} username={user.username} handleClick={this.selectUser} key={user}/>
								})
							}
						</ul>
					) || null
